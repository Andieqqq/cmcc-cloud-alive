...
const TLS_MINOR_VERSIONS = new Set([0x00, 0x01, 0x02, 0x03, 0x04]);
const TLS_MAX_RECORD_LENGTH = 0x4800;
...
function looksLikeTlsRecordAt(buffer, offset = 0) {
  ...
  if (!TLS_CONTENT_TYPES.has(buffer.readUInt8(offset))) return false;
  if (buffer.readUInt8(offset + 1) !== TLS_MAJOR_VERSION) return false;
  if (!TLS_MINOR_VERSIONS.has(buffer.readUInt8(offset + 2))) return false;
  const recordLength = buffer.readUInt16BE(offset + 3);
  return recordLength > 0 && recordLength <= TLS_MAX_RECORD_LENGTH;
}
...
function deriveZteCagReadyPlanFromConnectReply(input, opts = {}) {
  const raw = Buffer.isBuffer(input)
    ? input
    : Buffer.from(String(input || '').replace(/^0x/i, ''), 'hex');
  const controlWord = opts.controlWord === undefined ? 0x1405 : Number(opts.controlWord);
  const marker = raw.length > 8 ? raw.readUInt8(8) : null;
  ...
  return {
    known: false,
    source: observedCandidates.length
      ? 'observed_marker_has_multiple_or_unproven_ready_candidates'
      : 'unknown_connect_reply_marker',
    marker,
    markerHex: marker === null ? null : `0x${marker.toString(16).padStart(2, '0')}`,
    readyControlWord: controlWord,
    observedCandidates,
  };
}
...
function encodeZteCagPreflightProbeDatagram(opts = {}) {
  const probeBody = opts.probeBody
    ? Buffer.from(String(opts.probeBody).replace(/^0x/i, ''), 'hex')
    : Buffer.from('80020a0a1027', 'hex');
  if (probeBody.length !== 6) throw new Error('preflight probeBody must be 6 bytes hex');
  const echoTail = opts.echoTail
    ? Buffer.from(String(opts.echoTail).replace(/^0x/i, ''), 'hex')
    : crypto.randomBytes(14);
  if (echoTail.length !== 14) throw new Error('preflight echoTail must be 14 bytes hex');
  return Buffer.concat([encodeZteCagPacket(probeBody), echoTail]);
}
